...
- На долгосрочные визы D (Visiteur, Passeport Talent и пр.) — ответы и выдача слотов стабильнее, рассматривают в сроки 2–5 недель (бывают задержки), но если есть приглашение/контракт — слоты дают быстрее.\
...
- Если нет ответа 2 недели и дата поездки приближается — допускается отправить напоминание (в той же цепочке).
- Однако чрезмерное количество писем (спам-атаки раз в 2–7 дней, смена почтовых адресов) не увеличивает шансы, а лишь увеличивает нагрузку на консульство и может привести к игнору.
- Если слоты начали "раздавать" на вашу дату (см. сообщения в чате: часто появляются трекинги кто на какую дату получает), рекомендуется отправлять повторный запрос/reminder — были кейсы, когда после такого письма слот получали через сутки.
- Нет гарантии, что ответят на самое свежее письмо — зачастую отвечают на первое по датам письмо или вообще выборочно.
...
#### Биометрия
- Биометрия (отпечатки) сдаются в момент подачи документов при визите — отдельно записываться не надо. Если недавно уже сдавали (до 59 месяцев, 5 лет), заново брать не будут, **но бывают исключения**.
    - В 2025 году встречались кейсы, когда повторно не брали отпечатки, если предыдущая виза была выдана не более 2–3 лет назад и виза была французская.
    - Для подачи в РФ: биометрия, сданная в Сербии, может не быть в базе визового центра РФ (и наоборот); иногда могут потребовать сдать повторно, но чаще всего отпечатки шенгенские общие — см. детали в [FAQ](#биометрия-и-подача-в-других-странах).

- Детей 12 лет и старше обязательно приводить лично (биометрия). До 12 лет — чаще всего требуется физическое присутствие ребенка, даже младше, по требованиям сотрудников (см. свежие кейсы в чате). После 12 лет — отпечатки строго обязательны для первой подачи.




**Сроки рассмотрения (апдейт 2025):**
- Стандарт: 7–15 рабочих дней (1–3 недели, включая бумажное "лежание").
- В последние месяцы (зима–весна 2025) масса кейсов, когда паспорта с визой выдаются за 12–19 дней (в среднем 2 недели с подачи, иногда быстрее для поездок, которые вот-вот наступают).
- Бывают задержки — вплоть до 3–4 недель (особенно февраль–март 2025 по отзывам массово затянули сроки), иногда решение дают буквально за пару дней до вылета!
- Некоторые получают паспорта лишь за 1–2 дня до даты первой поездки (очень часто в марте–апреле 2025!).
- После смены статуса заявки в личном кабинете можно сразу забирать паспорт без отдельного письма.
- Если поездка вот-вот, а ответа нет — **допускается приехать в окошко выдачи паспортов и спросить**, иногда прямо при вас делают визу/вклейку (см. [FAQ](#чего-делать-если-виза-не-готова-а-вылет-скоро)).


**Важно:**

  - Для семьи совместной поездки обычно дают ОДИН слот на всех, если в письме указаны все заявители, ФИО, паспорта и т.д. — сообщайте полный список сразу! (Март 2025: те, кто пишут отдельные письма — могут быть записаны на разные даты, но в большинстве практик дают 1 слот — для семьи, супругов, детей на одного взрослого, если упомянуты в письме.)
  - Исключения: если ваш кейс не выглядит как семейная совместная поездка, или Вы не указали всех сразу.
  - Брать детей нужно по каждому случаю уточнять — для детей школьного возраста стандартно требуется их присутствие на первой подаче.
...
**Можно ли подаваться/записываться на новую визу, если старая еще не истекла?**
  - Да, если по старой уже была осуществлена поездка ("откатана"), и главное — чтобы визы по времени не пересекались с действующей ещё визой. Если в действующем паспорте стоит старая виза, которой уже воспользовались, можно подавать на новую визу — главное корректно указать прошлый опыт.
...
### FAQ по подаче и мультиподаче (семья/группа/друзья)
...
!!! note "Фото и дети"
    ...
    - Детей рекомендуется брать с собой на подачу (даже младенцы). После 12 лет — строго для биометрии. См. также [пример фото (pdf)](https://rs.ambafrance.org/IMG/pdf/depliant_norme_photo-2.pdf?7163/0d839475fdc9a69416061480110e23585460c82b) и [инструкцию консульства](https://rs.ambafrance.org/Visas-in-English).
...
**Выписка по счету:**
- **Нужна ли выписка по движению средств за 3 месяца (а не только остаток)?**
    - По практике 2024–2025 года — **да, обязательно** делайте выписку по движению средств за последние 3 месяца (стандартная банковская выписка на сербском или английском). Просто справки об остатке по счёту недостаточно — сотрудница часто требует показать именно движение (promet) за период до подачи.
    - Исключение: если вы уже получали французскую визу в Сербии не первый раз, и в списке на сайте нет требования движения, можно попробовать приложить только остаток, но шансы на возврат/запрос доноса возрастают.
    - Для ИП движение по бизнес-счету — **обязательно**; если есть личный счёт — хорошо приложить обе выписки.
    - Иногда просят сделать перевод сумм поступлений и баланса из рублей в евро прямо на справке/распечатке вручную! (Добавляйте, если указываете российские счета или доход).


!!! warning "Выписка по счёту — важные детали (2025)"
    - Выписки по счету обычно достаточно PDF/распечатки из интернет-банка (Райфайзен и др.), оригинал с печатью опционален. Крайне свежая справка: оптимально не старше 2–3 недель до даты подачи (лучше 1 неделя).
...
!!! tip "Страховка на визу и российские полисы: уточнения 2024-2025"
    - Страховка должна покрывать даты поездки.
    - Сербские страховки (Dunav, Uniqa и др.) — традиционно самый надежный выбор (делать можно онлайн, PDF подходит).
    - Российские страховки (Тинькофф, Т-банк и пр.): 
        - Французское консульство в Сербии большинство кейсов **принимает** такие страховки, визу выдают (было несколько кейсов, когда принимали полис Тинькофф – написано прямо в мае 2025). 
        - Но есть риск, что сотрудник или консул может запросить заменить страховку на сербскую (или отказать на этапе рассмотрения — аналогичное требование подтверждено у немцев!),
        - На практике лучше перестраховаться и оформить сербский полис, чтобы избежать требовать досыл.
    - Не оформляйте страховки от российских банков через агрегаторы, не имеющих международных лицензий (страховая обязательно должна принимать на территории Шенгена; в случае ЧП возможны проблемы).
    - На детей, при семейной подаче — один полис на всех подходит (в любом случае имя каждого члена указывается в таблице покрытия).
...
+ Для детей:
    - Для подачи с детьми младше 6 лет чаще всего требуется личное присутствие ребенка на подаче. Практика для детей до 12 лет может меняться — уточняйте в consulate/чате (много отзывов, что и на малышей требуют слоты, а иногда просят только фото и копии документов). Для старше 12 лет — обязательно отпечатки пальцев.
    - На практике чаще всего требуется присутствие детей, если это первая подача или новая биометрия.
    - Для подачи на визу несовершеннолетнему ребёнку, если оба родителя указаны в свидетельстве о рождении, дополнительно ничего не требуется, если оба подают документы и выезжают вместе.
    - Если ребёнок подается только с одним родителем или только по спонсорству, требуется нотариальная доверенность от отсутствующего родителя на английском или французском, судебный перевод; апостиль не нужен.
    - Для детей старше 7 лет — требуется личное присутствие на подаче; младше 7 лет — иногда принимают без присутствия, но рекомендуем уточнить в письме или на приёме (по отзывам 2024-2025 – требуют чаще присутствие для отпечатков и фото).
...
**Даты в анкете France-Visas:**
  ...
  - В среднем в 2025: дают либо 15/30, либо мульти на 30 дней/2 месяца, крайне редко — длинные (6–12 месяцев), еще реже — годовые визы (почти всегда по рабочему приглашению или для повторных бизнес-поездок).
...
**Срок действия боравка (ВНЖ) Сербии:**
  - На слоте главное — наличие ВНЖ на момент подачи/поездки. Если у вас ВНЖ на продлении, письма о сдаче на продление принимаются. 
  - Теоретически можно получить слот без ВНЖ (например, ожидая выдачи карты), по факту к подаче документы должны быть уже с действующим ВНЖ. Если вы получите ВНЖ к дате слота — это принимается, дата запроса значения не имеет.
...
- **Свежее трудоустройство** - менее 6 месяцев работы в Сербии
- **Свежее ИП** - менее 3 месяцев с момента открытия  
- **Нет связи с Сербией** - отсутствие работы, аренды, учебы детей
- **Больше дней в другой стране** - основное пребывание должно быть во Франции
